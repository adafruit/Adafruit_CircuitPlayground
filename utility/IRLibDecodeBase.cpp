...
void IRdecodeBase::dumpResults(bool verbose) {
  ...
  if ((protocolNum > 89) || (protocolNum <= LAST_PROTOCOL)) {
    ...
    Serial.print(Pnames(protocolNum));
    Serial.print(F("("));
    Serial.print(protocolNum, DEC);
  ...
  if (recvGlobal.didAutoResume)
    Serial.print(F("Auto Resumed"));
  Serial.println();
  if (!verbose)
    return;
  Serial.print(F("Raw samples("));
  Serial.print(recvGlobal.decodeLength, DEC);
  Serial.print(F("): Gap:"));
  Serial.println(recvGlobal.decodeBuffer[0], DEC);
  Serial.print(F("  Head: m"));
  Serial.print(recvGlobal.decodeBuffer[1], DEC);
  Serial.print(F("  s"));
  Serial.println(recvGlobal.decodeBuffer[2], DEC);
  int LowSpace = 32767;
  int LowMark = 32767;
  int HiSpace = 0;
  int HiMark = 0;
  ...
  for (i = 3; i < recvGlobal.decodeLength; i++) {
    ...
    if (i % 2) {
      LowMark = min(LowMark, interval);
      HiMark = max(HiMark, interval);
  ...
  Serial.println();
  Serial.print(F("Extent="));
  Serial.println(Extent, DEC);
  Serial.print(F("Mark  min:"));
  Serial.print(LowMark, DEC);
  Serial.print(F("\t max:"));
  Serial.println(HiMark, DEC);
  Serial.print(F("Space min:"));
  Serial.print(LowSpace, DEC);
  Serial.print(F("\t max:"));
  Serial.println(HiSpace, DEC);
  Serial.println();
}

/* We use a generic routine because most protocols have the same basic
 * structure. Previous versions of this method would handle protocols with
 * variable marks or variable spaces. However we have discovered that only Sony
 * protocol uses variable marks so we have stripped out that portion of the
 * code. This changes the number of necessary parameters. We no longer need
 * markOne and markZero because they are both the same which we will pass in
 * markData. Note this new version will handle up to 48 bits putting the most
 * significant 16 bits in "this.address" in the least significant 32 bits in
 * "this.data". We could have allowed for 64 bit but we have not seen generic
 * protocols that large.
 */
bool IRdecodeBase::decodeGeneric(uint8_t expectedLength, uint16_t headMark,
  ...
  // If "expectedLenght" or "headMark" or "headSpace" are zero or if
  // "ignoreHeader" is true then don't perform these tests. This is because some
  // protocols need to do their own custom header work.
  uint64_t data = 0;
  ...
  while (offset < Max) {
    ...
    if (MATCH(recvGlobal.decodeBuffer[offset], spaceOne)) {
      ...
      return DATA_SPACE_ERROR(spaceZero);
    offset++;
  }
