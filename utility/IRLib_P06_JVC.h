...
#define IR_SEND_PROTOCOL_06                                                    \
  case 06:                                                                     \
    IRsendJVC::send(data, (bool)data2);                                        \
    break;
#define IR_DECODE_PROTOCOL_06                                                  \
  if (IRdecodeJVC::decode())                                                   \
    return true;
...
public:
  void send(uint32_t data, bool first = true) {
    sendGeneric(data, 16, 525 * 16 * first, 525 * 8 * first, 525, 525, 525 * 3,
                525, 38, true);
    space(525 * 45);
    if (first) {
    ...
    if (!decodeGeneric(36, 525 * 16, 525 * 8, 525, 525 * 3, 525)) {
      ...
      if (recvGlobal.decodeLength == 34) {
        if (!decodeGeneric(0, 525, 0, 525, 525 * 3, 525)) {
          ...
          // If this is a repeat code then IRdecodeBase::decodeGeneric fails to
          // add the most significant bit
          if (MATCH(recvGlobal.decodeBuffer[2], (525 * 3))) {
            value |= 0x8000;
          } else {
            if (!MATCH(recvGlobal.decodeBuffer[2], 525))
              return DATA_SPACE_ERROR(525);
          }
        }
        bits++;
      } else
