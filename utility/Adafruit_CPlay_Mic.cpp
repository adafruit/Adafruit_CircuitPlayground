...
/**************************************************************************/
/*!
    @brief  Reads ADC for given interval (in milliseconds, 1-65535). Uses ADC
   free-run mode w/polling on AVR. Any currently-installed ADC interrupt handler
   will be temporarily disabled while this runs.
    @param ms the number of milliseconds to sample
    @return max deviation from DC_OFFSET (e.g. 0-341)
    @deprecated THIS FUNCTION IS DEPRECATED AND WILL BE REMOVED IN A FUTURE
   RELEASE. please use soundPressureLevel(ms) instead
...
/*!
    ...
    @note ON AVR: Captures ADC audio samples at maximum speed supported by 32u4
   (9615 Hz). Ostensibly for FFT code (below), but might have other uses.  Uses
   ADC free-run mode w/polling.  Any currently-installed ADC interrupt handler
...
void Adafruit_CPlay_Mic::capture(int16_t *buf, uint16_t nSamples) {
  ...
  while (ptr < (buf + nSamples)) {
    ...
    for (uint8_t samplenum = 0; samplenum < (DECIMATION / 16); samplenum++) {
      uint16_t sample =
          pdm.read() &
          0xFFFF; // we read 16 bits at a time, by default the low half

      ADAPDM_REPEAT_LOOP_16( // manually unroll loop: for (int8_t b=0; b<16;
                             // b++)
          {
            // start at the LSB which is the 'first' bit to come down the line,
            // chronologically (Note we had to set I2S_SERCTRL_BITREV to get
            // this to work, but saves us time!)
            if (sample & 0x1) {
    ...
    // since we wait for the samples from I2S peripheral, we dont need to delay,
    // we will 'naturally' wait the right amount of time between analog writes
...
/*!
    ...
    @param ms Milliseconds to continuously sample microphone over, 10ms is a
   good start.
    @returns Floating point Sound Pressure Level, tends to range from 40-120 db
   SPL
*/
/**************************************************************************/
...
extern "C" { // In ffft.S
void fft_input(const int16_t *, complex_t *), fft_execute(complex_t *),
    fft_output(complex_t *, uint16_t *);
}

/**************************************************************************/
/*!
    @brief  AVR ONLY: Performs one cycle of fast Fourier transform (FFT) with
   audio captured from mic on A4.  Output is 32 'bins,' each covering an equal
   range of frequencies from 0 to 4800 Hz (i.e. 0-150 Hz, 150-300 Hz, 300-450,
   etc). Needs about 450 bytes free RAM to operate.
    @param spectrum the buffer to store the results in. Must be 32 bytes in
   length.
