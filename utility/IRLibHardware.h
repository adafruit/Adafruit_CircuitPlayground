 ...
 *
 * This file was formally called IRLibTimer.h but we have renamed it because it
 * more accurately reflects the fact that it contains all of the hardware
 * specific defines. It contains mostly definitions for 8-bit AVR processors.
 * More advanced processors such as SAMD21 definitions will now be defined in a
 * separate processor specific header file. NOTE: IRLibHardware.cpp is not
 * related to hardware. See the comments in that file to explain why it was
 * created and what purpose it serves.
 */

/* This file has been significantly reconfigured for version 1.5 and greater. It
 * now allows you use a different timer for input versus output. This also
 * allows you to use no timers whatsoever by using the IRrecvPCI or IRrecvLoop
 * for input and bit-bang routines for output. Note the various "Teensy"
...
/* In this section we attempt to detect your hardware type and give you the
 * choice of various hardware timers to change the PWM frequency for output. In
 * each hardware section below you should un-comment only one of the choices.
 * The defines also specify the output pin number. DO NOT change the pin number
 * unless you're using non-standard hardware that would require you to do so. If
 * you wish to use the bit-bang PWM output, you will specify that AFTER this
 * section as an override.
 */
 ...
 * The definition must include the pin number for bit-bang output.  This could
 * be any available digital output pin. It need not be a designated PWM pin.
...
/* We are going to presume that you want to use the same hardware timer to
 * control the 50 microsecond interrupt used by the IRrecv receiver class as was
 * specified above in the hardware detection section for sending. Even if you
 * specified bit-bang for sending, the definitions above have selected a default
 * sending timer for you based on hardware detection. If that is correct, then
 * do nothing below.  However if you do wish to specify an IR_RECV_TIMER
 * different than the IR_SEND_TIMER selected by the code above, then you should
 * un-comment the IR_RECV_TIMER_OVERRIDE and also un-comment one and only one of
 * the following IR_RECV_TIMERx lines below that. NOTE: You are responsible for
 * ensuring that the timer you are specifying is available on your hardware. You
 * should only choose timers which are shown as available for your hardware
 * platform as shown in the defines in the IR_SEND_TIMER section above. NOTE:
 * This discussion does not apply to SAMD 21 processors.
 */
...
#define IR_SEND_PWM_START                                                      \
  ...
  for (unsigned int j = 0; j < jmax; j++) {                                    \
    digitalWrite(IR_SEND_BIT_BANG, HIGH);                                      \
    delayMicroseconds(onTime);                                                 \
    digitalWrite(IR_SEND_BIT_BANG, LOW);                                       \
    delayMicroseconds(offTime);                                                \
  }
#define IR_SEND_MARK_TIME(time)
#define IR_SEND_PWM_STOP
#define IR_SEND_CONFIG_KHZ(val)                                                \
  float Length = 1000.0 / (float)khz;                                          \
  iLength = int(Length + 0.5);                                                 \
  onTime = int(Length / 3.0);                                                  \
...
#elif defined(                                                                 \
    IR_SEND_TIMER4_HS) // defines for timer4 (10 bits, high speed option)
 ...
 * earlier in this file.  Theoretically you could change the 50 but it has not
 * been tested.
 */
...
#define IR_RECV_CONFIG_TICKS()                                                 \
  ({                                                                           \
    ...
    OCR1A = SYSCLOCK * USEC_PER_TICK / 1000000;                                \
    TCNT1 = 0;                                                                 \
  })
...
#define IR_RECV_CONFIG_TICKS()                                                 \
  ({                                                                           \
    ...
    OCR3A = SYSCLOCK * USEC_PER_TICK / 1000000;                                \
    TCNT3 = 0;                                                                 \
  })

#elif defined(                                                                 \
    IR_RECV_TIMER4_HS) // defines for timer4 (10 bits, high speed option)
...
#define IR_RECV_CONFIG_TICKS()                                                 \
  ({                                                                           \
    ...
    OCR4A = SYSCLOCK * USEC_PER_TICK / 1000000;                                \
    TCNT4 = 0;                                                                 \
  })
...
#define IR_RECV_CONFIG_TICKS()                                                 \
  ({                                                                           \
    ...
    OCR5A = SYSCLOCK * USEC_PER_TICK / 1000000;                                \
    TCNT5 = 0;                                                                 \
  })
