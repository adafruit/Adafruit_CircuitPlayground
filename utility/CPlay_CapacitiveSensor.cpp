...
*/
/**************************************************************************/
CPlay_CapacitiveSensor::CPlay_CapacitiveSensor(uint8_t sendPin,
                                               uint8_t receivePin) {
  ...
  CS_Timeout_Millis =
      (2000 * (float)loopTimingFactor * (float)F_CPU) / 16000000;
  ...
  if ((sendPin != 255) && (sendPin >= NUM_DIGITAL_PINS))
    error = -3;
  ...
  // get pin mapping and port for receive Pin - from digital pin functions in
  // Wiring.c
...
long CPlay_CapacitiveSensor::capacitiveSensor(uint8_t samples) {
  ...
  for (uint8_t i = 0; i < samples;
       i++) { // loop for samples parameter - simple lowpass filter
    if (SenseOneCycle() < 0)
      return -2; // variable over timeout
  }

  // only calibrate if time is greater than CS_AutocaL_Millis and total is less
  // than 10% of baseline this is an attempt to keep from calibrating when the
  // sensor is seeing a "touched" signal
...
void CPlay_CapacitiveSensor::set_CS_Timeout_Millis(
  ...
  CS_Timeout_Millis =
      (timeout_millis * (float)loopTimingFactor * (float)F_CPU) /
      16000000; // floats to deal with large numbers
}
...
int CPlay_CapacitiveSensor::SenseOneCycle(void) {
  if (send_mask != 0) {
    ...
    *send_outport |= send_mask;    // sendPin Register high
    ...
    while (!(*recv_inport & recv_mask) &&
           (total < CS_Timeout_Millis)) { // while receive pin is LOW AND total
                                          // is positive value
      total++;
    }
  ...
  // set receive pin HIGH briefly to charge up fully - because the while loop
  // above will exit when pin is ~ 2.5V
  noInterrupts();
  *send_outport &= ~send_mask;  // sendPin Register low
  *recv_direction |= recv_mask; // receivePin to OUTPUT
