 ...
 */
/*
 * Sony is backwards from most protocols. It uses a variable length mark and a
 * fixed length space rather than a fixed mark and a variable space. Our generic
 * send will still work however we need a custom decoding routine because it's
 * difficult to get the generic decoder to handle a variable length mark without
 * cluttering up the code to much. According to the protocol you must send Sony
 * commands at least three times so we automatically do it here. Sony can be 8,
 * 12, 15, or 20 bits in length. The 8 bit version uses a shorter trailing space
 * at the end. The signal is modulated at 40 kHz however most 38 kHz receivers
 * are broad enough to receive it.
 */
...
#define IR_SEND_PROTOCOL_02                                                    \
  case 2:                                                                      \
    IRsendSony::send(data, data2);                                             \
    break;
#define IR_DECODE_PROTOCOL_02                                                  \
  if (IRdecodeSony::decode())                                                  \
    return true;
...
public:
  virtual bool decode(void) {
    ...
    if (recvGlobal.decodeLength != 2 * 8 + 2 &&
        recvGlobal.decodeLength != 2 * 12 + 2 &&
        recvGlobal.decodeLength != 2 * 15 + 2 &&
        recvGlobal.decodeLength != 2 * 20 + 2)
      return RAW_COUNT_ERROR;
    if (!ignoreHeader) {
      if (!MATCH(recvGlobal.decodeBuffer[1], 600 * 4))
        return HEADER_MARK_ERROR(600 * 4);
    }
