 ...
 */
/*
 * This module implements the protocol used by DirecTV. It comes in six
 * different varieties. It uses three different frequencies 38, 40, or 57 kHz.
 * It also uses two different varieties lead out times either 9000us or 30000us.
 * The default is 38 kHz and 30000us. Because the decoder does not usually
 * detect frequency and does not record the lead out time, you may have
 * difficulty determining which variety your device uses.
 *
 ...
 * and length 600us=logical 0. So whereas the normal protocol requires both a
 * mark and a space to encode a single bit, this protocol encodes one bit in
 * each mark and space.  It also makes changes to the length of the header mark
 * to devote repeat codes. The first header mark should be 6000us but repeat
 * codes should only be 3000us. The decode routine sets "address=true" if it is
 * a first and "address=false" otherwise.
 */
...
#define IR_SEND_PROTOCOL_10                                                    \
  case 10:                                                                     \
    IRsendDirecTV::send(data, data2, khz);                                     \
    break;
#define IR_DECODE_PROTOCOL_10                                                  \
  if (IRdecodeDirecTV::decode())                                               \
    return true;
...
public                                                                         \
  virtual IRdecodeDirecTV
#define PV_IR_SEND_PROTOCOL_10                                                 \
public                                                                         \
  virtual IRsendDirecTV
#endif
...
public:
  bool decode(void) {
    ...
    if (!MATCH(recvGlobal.decodeBuffer[2], 1200))
      return HEADER_SPACE_ERROR(1200);
    uint32_t data = 0;
    offset = 3;
    while (offset < 18) {
