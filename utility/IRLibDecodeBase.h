 ...
 */
/*
 * This module contains the base classes for decoding. You will not create
 * instances of these classes, rather you will use them as base classes in
 * creating derived protocol specific decoders.
 */
...
public:
  ...
  bool decodeGeneric(uint8_t expectedLength, uint16_t headMark,
                     uint16_t headSpace, uint16_t markData, uint16_t spaceOne,
                     uint16_t spaceZero);
 ...
 * used internally. They used to be in a separate file IRLibMatch.h but it's
 * easier to include them here.  You need not worry about them unless you are
 * creating your own decoding routines. See the documentation how to implement
 * new protocols for a more detailed explanation of these definitions and
 * routines.
 */

/*
 * Originally all timing comparisons for decoding were based on a percent of the
 * target value. However when target values are relatively large, the percent
 * tolerance is too much.  In some instances an absolute tolerance is needed. In
 * order to maintain backward compatibility, the default will be to continue to
 * use percent. If you wish to default to an absolute tolerance, you should
 * comment out the line below.
 */
#define IRLIB_USE_PERCENT

/*
 * These are some miscellaneous definitions that are needed by the decoding
 * routines. You need not include this file unless you are creating custom
 * decode routines which will require these macros and definitions. Even if you
 * include it, you probably don't need to be intimately familiar with the
 * internal details.
 */
...
uint8_t IRLIB_REJECTION_MESSAGE(const __FlashStringHelper *s) {
  Serial.print(F(" Protocol failed because "));
  Serial.print(s);
  Serial.println(F(" wrong."));
  return false;
};
uint8_t IRLIB_DATA_ERROR_MESSAGE(const __FlashStringHelper *s, uint8_t index,
                                 uint16_t value, uint16_t expected) {
  IRLIB_REJECTION_MESSAGE(s);
  Serial.print(F("Error occurred with decodeBuffer["));
...
#define HEADER_MARK_ERROR(expected)                                            \
  IRLIB_DATA_ERROR_MESSAGE(F("header mark"), 1, recvGlobal.decodeBuffer[1],    \
                           expected);
#define HEADER_SPACE_ERROR(expected)                                           \
  IRLIB_DATA_ERROR_MESSAGE(F("header space"), 2, recvGlobal.decodeBuffer[2],   \
                           expected);
#define DATA_MARK_ERROR(expected)                                              \
  IRLIB_DATA_ERROR_MESSAGE(F("data mark"), offset,                             \
                           recvGlobal.decodeBuffer[offset], expected);
#define DATA_SPACE_ERROR(expected)                                             \
  IRLIB_DATA_ERROR_MESSAGE(F("data space"), offset,                            \
                           recvGlobal.decodeBuffer[offset], expected);
#define TRAILER_BIT_ERROR(expected)                                            \
  IRLIB_DATA_ERROR_MESSAGE(F("RC6 trailer bit length"), offset,                \
                           recvGlobal.decodeBuffer[offset], expected);
