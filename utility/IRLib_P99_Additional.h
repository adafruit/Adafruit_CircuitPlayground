 ...
 */
/*
 * This is dummy code that you can copy and rename and modify when implementing
 * new protocols.
 */

#ifndef IRLIB_PROTOCOL_99_H
#define IRLIB_PROTOCOL_99_H
#define IR_SEND_PROTOCOL_99                                                    \
  case 99:                                                                     \
    IRsendAdditional::send(data);                                              \
    break;
#define IR_DECODE_PROTOCOL_99                                                  \
  if (IRdecodeAdditional::decode())                                            \
    return true;
...
#else
#define PV_IR_DECODE_PROTOCOL_99                                               \
public                                                                         \
  virtual IRdecodeAdditional
#define PV_IR_SEND_PROTOCOL_99                                                 \
public                                                                         \
  virtual IRsendAdditional
#endif
...
public:
  void IRsendAdditional::send(uint32_t data){
      // void IRsendAdditional::send(uint32_t data, uint32_t data2)//optional
      // form
      /*********
