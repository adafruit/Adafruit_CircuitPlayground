/* IRLib_P12_CYKM.h
  ...
  See COPYRIGHT.txt and LICENSE.txt for license information (spoilers it's GPL
  3)
...
#define IR_SEND_PROTOCOL_12                                                    \
  case 12:                                                                     \
    IRsendCYKM::send(data);                                                    \
    break;
#define IR_DECODE_PROTOCOL_12                                                  \
  if (IRdecodeCYKM::decode())                                                  \
    return true;
...
public:
  bool decode(void) {
    ...
    if (!decodeGeneric(34, 3100, 3100, 650, 650 * 3, 650))
      return false;
    protocolNum = CYKM;
    parseValue();
    return true;
  ...
  virtual void dumpResults(void) {
    Serial.print(F("Gap="));
    Serial.print(recvGlobal.decodeBuffer[0], DEC);
    Serial.print(F(" CYKM decoded:"));
    ...
    case CYKM_MOUSE_HOLD:
      ...
      showMods();
      switch (cmdData) {
      case CYKM_LEFT_BUTTON:
        Serial.print(F("left"));
        break;
      case CYKM_RIGHT_BUTTON:
        Serial.print(F("right"));
        break;
      case CYKM_MIDDLE_BUTTON:
    ...
    case CYKM_MOUSE_MOVE:
      if (cmdData &
          (CYKM_DIR_RIGHT | CYKM_DIR_LEFT | CYKM_DIR_UP | CYKM_DIR_DOWN)) {
        Serial.print(F("Mouse move "));
        if (cmdData & CYKM_DIR_RIGHT)
          Serial.print(F("right "));
        else if (cmdData & CYKM_DIR_LEFT)
          Serial.print(F("left "));
        if (cmdData & CYKM_DIR_UP)
          Serial.print(F("up "));
        else if (cmdData & CYKM_DIR_DOWN)
          Serial.print(F("down "));
      } else {
        if (cmdData & (CYKM_WHEEL_UP | CYKM_WHEEL_DOWN)) {
          ...
          if (cmdData & CYKM_SPEED_DECREASE) {
            ...
            if (cmdData & CYKM_SPEED_INCREASE) {
              ...
              Serial.println(F("Mouse Move Error"));
          }
        };
      };
    ...
    case CYKM_TOGGLE:
      ...
      if (cmdData == CYKM_TOGGLE_RESET) {
        ...
        if (cmdData & CYKM_TOGGLE_LEFT)
          Serial.print(F(" left button"));
        if (cmdData & CYKM_TOGGLE_RIGHT)
          Serial.print(F(" right button"));
        if (cmdData & CYKM_TOGGLE_MIDDLE)
          Serial.print(F(" middle button"));
        if (cmdData & CYKM_TOGGLE_SHIFT)
          Serial.print(F(" shift key"));
        if (cmdData & CYKM_TOGGLE_CONTROL)
          Serial.print(F(" control key"));
        if (cmdData & CYKM_TOGGLE_ALT)
          Serial.print(F(" alt key"));
        if (cmdData & CYKM_TOGGLE_GUI)
          Serial.print(F(" GUI"));
      }
      Serial.print(F(" Bits= "));
      Serial.println(toggleData, HEX);
      break;
    case CYKM_KEY_WRITE:
    case CYKM_KEY_HOLD:
      if (cmdType == CYKM_KEY_WRITE) {
        Serial.print(F("Key write"));
        showMods();
      } else {
      ...
      Serial.print("  ");
      Serial.print(cmdData, DEC);
      if ((cmdData > 31) && (cmdData < 128)) {
        Serial.print(" '");
      ...
      break;
    case CYKM_SPEED:
      Serial.print(F("Change speed to:"));
      Serial.println(cmdData, DEC);
      break;
    case CYKM_CUSTOM_DEVICE:
      Serial.print(F("Custom Device data:"));
  ...
  void doMouseKeyboard(void) {
    ...
    case CYKM_MOUSE_MOVE:
      ...
      if (cmdData & CYKM_DIR_UP)
        ...
        yDir = mouseSpeed;
      if (cmdData & CYKM_WHEEL_UP)
        wDir = 1;
      else if (cmdData & CYKM_WHEEL_DOWN)
        wDir = -1;
      if (xDir || yDir || wDir) {
        Mouse.move(xDir, yDir, wDir);
      } else {
        if ((cmdData & CYKM_SPEED_DECREASE) && (mouseSpeed > 0))
          mouseSpeed--;
        if ((cmdData & CYKM_SPEED_INCREASE) && (mouseSpeed <= 255))
          mouseSpeed++;
      };
      break;
    case CYKM_TOGGLE:
      if (cmdData == CYKM_TOGGLE_RESET) {
        ...
        if (cmdData & CYKM_TOGGLE_LEFT) {
          if (toggleData & CYKM_TOGGLE_LEFT)
            Mouse.release(MOUSE_LEFT);
          else
            Mouse.press(MOUSE_LEFT);
        };
        if (cmdData & CYKM_TOGGLE_RIGHT) {
          if (toggleData & CYKM_TOGGLE_RIGHT)
            Mouse.release(MOUSE_RIGHT);
          else
            Mouse.press(MOUSE_RIGHT);
        };
        if (cmdData & CYKM_TOGGLE_MIDDLE) {
          if (toggleData & CYKM_TOGGLE_MIDDLE)
            Mouse.release(MOUSE_MIDDLE);
          else
            Mouse.press(MOUSE_MIDDLE);
        };
        if (cmdData & CYKM_TOGGLE_SHIFT) {
          if (toggleData & CYKM_TOGGLE_SHIFT)
            Keyboard.release(KEY_LEFT_SHIFT);
          else
            Keyboard.press(KEY_LEFT_SHIFT);
        };
        if (cmdData & CYKM_TOGGLE_CONTROL) {
          if (toggleData & CYKM_TOGGLE_CONTROL)
            Keyboard.release(KEY_LEFT_CTRL);
          else
            Keyboard.press(KEY_LEFT_CTRL);
        };
        if (cmdData & CYKM_TOGGLE_ALT) {
          if (toggleData & CYKM_TOGGLE_ALT)
            Keyboard.release(KEY_LEFT_ALT);
          else
            Keyboard.press(KEY_LEFT_ALT);
        };
        if (cmdData & CYKM_TOGGLE_GUI) {
          if (toggleData & CYKM_TOGGLE_GUI)
