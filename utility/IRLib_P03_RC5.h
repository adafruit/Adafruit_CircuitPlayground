 ...
 */
/*
 * The RC5 protocol was invented by Phillips but is used by wide variety of
 * manufacturers. It uses a phase encoding of data bits. A space/mark pair
 * indicates "1" and a mark/space indicates a "0". It begins with a single "1"
 * bit which is not encoded in the data. The second highest order data bit is a
 * toggle bit that indicates individual keypresses. You must toggle this bit
 * yourself when sending data. The protocol uses 36 kHz modulation however 38
 * kHz receivers can typically receive the codes okay. There are three supported
 * varieties as follows: RC5          13 bits, 36 kHz (default, most common
 * variety. RC5-F7       14 bits, 36 kHz RC5-F7-57    14 bits, 57 kHz There's
 * also a 19 bit variety called RC5x that is not supported here but may be a
 * separate module eventually.
 */
...
#define IR_SEND_PROTOCOL_03                                                    \
  case 03:                                                                     \
    if (khz == 38)                                                             \
      khz = 36;                                                                \
    IRsendRC5::send(data, data2, khz);                                         \
    break;
#define IR_DECODE_PROTOCOL_03                                                  \
  if (IRdecodeRC5::decode())                                                   \
    return true;
...
public:
  void send(uint32_t data, uint8_t nBits = 13, uint8_t kHz = 36) {
    ...
    // with patterns produced by the original library. Uncomment the following
    // two lines to maintain backward compatibility. space(RC5_T1); // Second
    // start bit mark(RC5_T1); // Second start bit
...
public:
  virtual bool decode(void) {
    ...
    if (recvGlobal.decodeLength < 13)
      return RAW_COUNT_ERROR;
    offset = 1; // Skip gap space
    data = 0;
    used = 0;
    // Get start bits
    if (getRClevel(&used, RC5_T1) != MARK)
      return HEADER_MARK_ERROR(RC5_T1);
    ...
    // HEADER_SPACE_ERROR(RC5_T1); if (getRClevel(&used, RC5_T1) != MARK) return
    // HEADER_MARK_ERROR(RC5_T1);
