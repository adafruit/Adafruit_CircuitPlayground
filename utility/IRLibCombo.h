 ...
 * the protocols that you will actually use. At the top of your sketch you
 *should include the Send and/or Decode base modules followed by at least one or
 *more protocol module. Then conclude with this module. WARNING: The lowest
 *numbered protocol which you are using MUST be included first. The remaining
 *protocol modules technically could be in any order however we recommend that
 *you maintain numerical order because you might at some point comment out the
 *top one and then the lowest would not be first causing an error.
 *
 * Here is an example...
 *
...
// Note protocol 90- 99 for sample code that will be unsupported in the final
// version. See IRLibProtocols/unsupported/IRLib_P90_Unsupported.h for details
...
#endif
/*
 * You may add additional protocols by copying and modifying the previous six
 * lines. You must also add appropriate macros in each segment below. Be sure to
 * maintain numerical order. Also the final entry in each list MUST BE the
 * Hash_Raw version.
 */
#ifdef IRLIBDECODEBASE_H
class IRdecode
    : PV_IR_DECODE_PROTOCOL_01 PV_IR_DECODE_PROTOCOL_02 PV_IR_DECODE_PROTOCOL_03
          PV_IR_DECODE_PROTOCOL_04 PV_IR_DECODE_PROTOCOL_05
              PV_IR_DECODE_PROTOCOL_06 PV_IR_DECODE_PROTOCOL_07
                  PV_IR_DECODE_PROTOCOL_08 PV_IR_DECODE_PROTOCOL_09
                      PV_IR_DECODE_PROTOCOL_10 PV_IR_DECODE_PROTOCOL_11
                          PV_IR_DECODE_PROTOCOL_12 PV_IR_DECODE_PROTOCOL_13
                              PV_IR_DECODE_PROTOCOL_14 PV_IR_DECODE_PROTOCOL_90 // Add additional 15, 16 etc. above this
...
public:
  bool decode(void) {
    ...
    IR_DECODE_PROTOCOL_90 // Add additional 15, 16 etc. above this
        IR_DECODE_PROTOCOL_91 IR_DECODE_PROTOCOL_92
...
class IRsend
    : PV_IR_SEND_PROTOCOL_01 PV_IR_SEND_PROTOCOL_02 PV_IR_SEND_PROTOCOL_03
          PV_IR_SEND_PROTOCOL_04 PV_IR_SEND_PROTOCOL_05 PV_IR_SEND_PROTOCOL_06
              PV_IR_SEND_PROTOCOL_07 PV_IR_SEND_PROTOCOL_08
                  PV_IR_SEND_PROTOCOL_09 PV_IR_SEND_PROTOCOL_10
                      PV_IR_SEND_PROTOCOL_11 PV_IR_SEND_PROTOCOL_12
  ...
  void send(uint8_t protocolNum, uint32_t data, uint16_t data2 = 0,
    ...
    switch (protocolNum) {
      ...
      IR_SEND_PROTOCOL_90 // Add additional 15, 16 etc. above this
          IR_SEND_PROTOCOL_91 IR_SEND_PROTOCOL_92
