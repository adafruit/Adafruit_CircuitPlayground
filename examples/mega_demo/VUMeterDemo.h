// This demo is based on the vumeter demo in the Adafruit Circuit Playground
// library.
  ...
  virtual void loop() {
    ...
    for (int i = 0; i <= numPixels - 1; i++) {
      CircuitPlayground.strip.setPixelColor(
          i, CircuitPlayground.colorWheel(map(i, 0, numPixels - 1, 30, 150)));
    }
    ...
    if (c <= numPixels) { // Fill partial column with off pixels
      drawLine(numPixels, numPixels - c,
               CircuitPlayground.strip.Color(0, 0, 0));
    }

    // Set the peak dot to match the rainbow gradient
    y = numPixels - peak;
    CircuitPlayground.strip.setPixelColor(
        y - 1, CircuitPlayground.colorWheel(map(y, 0, numPixels - 1, 30, 150)));
