...
#define TONE_DURATION_MS                                                       \
  100 // Duration in milliseconds to play a tone when touched.

class CapTouchDemo : public Demo {
public:
  uint16_t CAP_THRESHOLD =
      200; // Threshold for a capacitive touch (higher = less sensitive).
  ...
  virtual void loop() {
    ...
    // Check if any of the cap touch inputs are pressed and turn on those
    // pixels. Also play a tone if in tone playback mode.
    if (CircuitPlayground.readCap(0, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          3, CircuitPlayground.colorWheel(256 / 10 * 3));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(1, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          4, CircuitPlayground.colorWheel(256 / 10 * 4));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(2, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          1, CircuitPlayground.colorWheel(256 / 10));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(6, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          6, CircuitPlayground.colorWheel(256 / 10 * 6));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(9, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          8, CircuitPlayground.colorWheel(256 / 10 * 8));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(10, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          9, CircuitPlayground.colorWheel(256 / 10 * 9));
      if (playSound) {
    ...
    if (CircuitPlayground.readCap(12, CAP_SAMPLES) >= CAP_THRESHOLD) {
      CircuitPlayground.strip.setPixelColor(
          5, CircuitPlayground.colorWheel(256 / 10 * 5));
      if (playSound) {
