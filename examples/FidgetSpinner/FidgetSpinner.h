...
// so it can map to CircuitPlayground pixel positions easily.// Author: Tony
// DiCola License: MIT License (https://opensource.org/licenses/MIT) Usage:
// - Create an instance of the class (specifying a custom decay value between 0
// and 1,
//   the lower the decay the faster the spinner slows down).
// - Call spin to start the spinner moving at the specified velocity (in
// pixels/second).
// - Call getPosition periodically to get the current spinner position given an
// elapsed
...
public:
  ...
  void spin(float velocity) {
    // Start the fidget spinner moving at the specified velocity (in
    // pixels/second). Over time the spinner will slow down based on its decay
    // value.
    _velocity = velocity;
    _elapsedS =
        0.0; // Reset elapsed time to start the decay from the beginning.
  }

  float getPosition(float deltaS) {
    // Get the fidget spinner's current position after moving for the specified
    // number of milliseconds.  Returns a continuous value in the range 0...<10.
    _elapsedS += deltaS;
